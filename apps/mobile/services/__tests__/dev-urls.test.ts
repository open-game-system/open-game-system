...
import { addRecentDevUrl, getRecentDevUrls } from "../dev-urls";
...
describe("dev-urls", () => {
  ...
  describe("addRecentDevUrl", () => {
    it("adds URL to the front of the list", async () => {
      mockedAsyncStorage.getItem.mockResolvedValue(JSON.stringify(["https://old.dev"]));
      await addRecentDevUrl("https://new.dev");
      const stored = JSON.parse(mockedAsyncStorage.setItem.mock.calls[0][1] as string);
    ...
    it("deduplicates existing URLs by moving to front", async () => {
      ...
      const stored = JSON.parse(mockedAsyncStorage.setItem.mock.calls[0][1] as string);
      expect(stored).toEqual(["https://b.dev", "https://a.dev", "https://c.dev"]);
    });

    it("limits to 10 URLs", async () => {
      const tenUrls = Array.from({ length: 10 }, (_, i) => `https://url${i}.dev`);
    ...
    it("adds to empty list and uses correct storage key", async () => {
      ...
      expect(mockedAsyncStorage.getItem).toHaveBeenCalledWith("@ogs/recent_dev_urls");
