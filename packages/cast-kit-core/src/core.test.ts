...
import { isOGSCastAvailable, getCastState, onCastStateChange, dispatchCastEvent, getCastBridge, CAST_INITIAL_STATE, _resetBridge } from './core';
...
describe('Cast-Kit Core (Bridge-backed)', () => {
  ...
  describe('onCastStateChange', () => {
    ...
    it('subscribes to cast store state changes', () => {
      ...
      onCastStateChange((state) => { latestState = state; });
      ...
      window.dispatchEvent(new MessageEvent('message', {
        data: JSON.stringify({
          ...
          operations: [
            ...
            { op: 'add', path: '/devices/0', value: { id: 'tv-1', name: 'TV 1', type: 'chromecast' } },
          ],
        }),
      }));
    ...
    it('tracks device appearing via STATE_UPDATE', () => {
      ...
      window.dispatchEvent(new MessageEvent('message', {
        data: JSON.stringify({
          ...
          operations: [
            { op: 'replace', path: '/isAvailable', value: true },
            { op: 'add', path: '/devices/0', value: { id: 'tv-1', name: 'Living Room TV', type: 'chromecast' } },
          ],
        }),
      }));
    ...
    it('tracks error state transition via STATE_UPDATE', () => {
      ...
      window.dispatchEvent(new MessageEvent('message', {
        data: JSON.stringify({
          ...
          storeKey: 'cast',
          operations: [
            { op: 'replace', path: '/error', value: 'Failed to send state update' },
          ],
        }),
      }));
    ...
    it('tracks session connecting then connected', () => {
      ...
      window.dispatchEvent(new MessageEvent('message', {
        data: JSON.stringify({
          ...
          storeKey: 'cast',
          operations: [
            { op: 'replace', path: '/session/status', value: 'connecting' },
          ],
        }),
      }));
