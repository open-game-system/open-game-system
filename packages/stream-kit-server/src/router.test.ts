...
import { createMockHooks, type MockEnv } from "./mock-hooks";
...
describe("createStreamKitRouter", () => {
  ...
  describe("GET /stream/:streamId", () => {
    it("should return 404 when stream not found", async () => {
      const response = await handler(new Request("http://localhost/stream/123"), env);
  ...
  describe("GET /stream/:streamId/sse", () => {
    it("should establish SSE connection and send initial state", async () => {
      ...
      const response = await handler(new Request("http://localhost/stream/123/sse"), env);
      expect(response.status).toBe(200);
    ...
    it("should close SSE connection when stream does not exist", async () => {
      const response = await handler(new Request("http://localhost/stream/nonexistent/sse"), env);
    ...
    it("should handle multiple subscribers", async () => {
      ...
      const response1 = await handler(new Request("http://localhost/stream/123/sse"), env);
      const response2 = await handler(new Request("http://localhost/stream/123/sse"), env);
  ...
  describe("Error handling", () => {
    it("should return 404 for invalid paths", async () => {
      const response = await handler(new Request("http://localhost/invalid"), env);
      expect(response.status).toBe(404);
    });
    ...
    it("should handle hook errors gracefully", async () => {
      ...
      vi.spyOn(mockHooks, "loadStreamState").mockRejectedValueOnce(new Error("Test error"));
