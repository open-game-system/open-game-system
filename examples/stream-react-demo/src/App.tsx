...
import type {
  CreateRenderStreamParams,
  RenderStream,
  StreamClient,
} from "@open-game-system/stream-kit-web";
...
const createMockRenderStreamForTest = (params: CreateRenderStreamMethodParams): RenderStream => {
  ...
  if (params.autoConnect) {
    queueMicrotask(() => {
      if (currentState.status === "connecting") void stream.start();
    });
  }
  return stream;
};
...
const mockClientForTest: StreamClient = {
  ...
  createRenderStream: (params: CreateRenderStreamMethodParams): RenderStream =>
    createMockRenderStreamForTest(params),
...
const client: StreamClient = import.meta.env.VITEST
  ? mockClientForTest
  : createRealStreamClient({
        ...
        <div className="video-wrapper">
          {streamState.status === "streaming" && currentVideoElement ? (
            ...
            <div
              ...
              style={{ backgroundColor: "#222", width: "100%", height: "480px" }}
