...
import { getSessionCount, incrementSessionCount, shouldShowSwipeHint } from "../session-counter";
...
describe("session-counter", () => {
  describe("getSessionCount", () => {
    it("returns 0 when no count stored", async () => {
      ...
      expect(mockedAsyncStorage.getItem).toHaveBeenCalledWith("@ogs/session_count");
    });
  ...
  describe("incrementSessionCount", () => {
    it("increments from 0 to 1", async () => {
      ...
      expect(mockedAsyncStorage.setItem).toHaveBeenCalledWith("@ogs/session_count", "1");
    });
    ...
    it("increments existing count", async () => {
      ...
      expect(mockedAsyncStorage.setItem).toHaveBeenCalledWith("@ogs/session_count", "5");
    });
  });
